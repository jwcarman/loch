 ...
 *
 * <p>The ceiling is still checked, and still may depend on who is asking -- an sink is a door, not
 * a bypass. What changed is that reaching the door at all is no longer something a caller can
 * decide for itself.
 *
 ...
 *
 * <p>It also moves the type check left. {@code SurrogateSink<Invoice>.read(handleToSomeMail)} does
 * not compile, where naming a destination at a call site accepted any handle and refused at
 * runtime. The runtime check remains and is not redundant: a {@link String} that arrived as text
 * can be given any type by {@link Surrogate#of}, so what the store actually wrote is still the only
 * ground truth, and it is still what gets compared.
 *
 * @param <T> the type of value this sink will hand over, and no other
 */
